...
import input from "./input-ex.txt";
import { comparaCartas } from "./utils.ts"
...
export function day7part1() {
    ...
    manos.sort((a, b) => {
        return comparaCartas(a.cartas, b.cartas)
    })

    console.log(manos)
    let total = 0
    manos.forEach((value, index) => {
        // console.log(value.cartas, value.oferta, index + 1, value.oferta * (index + 1))
        total += value.oferta * (index + 1)
    })
    console.log(total)
}
